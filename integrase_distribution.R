...
## integrase number ====
# import only integrase data from pangenome
# edit pangenome data to binary data

integrase <- read_tsv(file = str_c(file_path, "integrase_pangenome.tsv")) %>% 
  select(-c(2:14)) %>% 
...
## phylogenic tree ====
ono_tree <- read.tree(file = "DATA/pangenome_analysis/775strains_tree_20200227.nwk") # phylogenic tree
...
### color ====
integrase_type <- c("+", "-")
integrase_color <- c("#005aff", "#c8c8cb")

### make data for heatmap ====
integrase_data <- integrase %>% 
  select(-strain) %>% 
  mutate_all(.funs = function(x){case_when(x == 0 ~ "-", TRUE ~ "+")}) %>% 
  data.frame(row.names = integrase$strain, check.names = FALSE, stringsAsFactors = FALSE)

#### integrase ====
hm_offset_3 = hm_offset_2 + (.01 * 1.2)
integrase_tree <- gheatmap(p = source_tree, data = integrase_data, 
                         offset = hm_offset_3, width = .03 * ncol(integrase_data), color = NULL, 
                         colnames = T, colnames_position = "bottom", hjust = 0,
                         colnames_offset_y = 5, font.size = fontsize_AMR) + 
  scale_fill_manual(name = "Integrase distribution", values = integrase_color, 
                    breaks = integrase_type, guide = guide_legend(order = 3)) + 
  new_scale_fill()

# output ====
letter_plot <- integrase_tree +
  theme(plot.margin = unit(c(0, .5, 0, 0), units = "line"),
...
ggsave(filename = str_c(file_path, "SF06_integrase_distribution.pdf"), plot = letter_plot, 
